____________________________________________________________________________Best Solution(Union Found)____________________________________________________________
class Solution {
    public int[] hitBricks(int[][] grid, int[][] hits) {
        ...
        for (int i = 0; i < M; i++) {
            for (int j = 0; j < N; j++) {
                ...
                addOneBrick(grid, ufs, num, i, j, M, N, false);
            }
        }
        ...
        for (int k = K - 1; k >= 0; k--) {
            ...
            else ret[k] = addOneBrick(grid, ufs, num, x, y, M, N, true);
        }
        return ret;
    }
    
    private int addOneBrick(int[][] grid, int[] ufs, int[] num, int x, int y, int M, int N, boolean union4) {
        ...
        count--;                        // remove the added brick itself if we have some bricks fixed. 
    ...
    private int union(int[] ufs, int[] num, int k1, int k2, int N) {
        ...
        // Only if some out-ceiling nodes has been unioned to ceiling set, we return the number of bricks settled. 
